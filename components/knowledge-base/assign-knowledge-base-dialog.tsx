...
interface AssignKnowledgeBaseDialogProps {
  isOpen: boolean
  onClose: () => void
  onAssign: (knowledgeBase: { id: string; name: string; description: string; dateAdded: Date }) => void
}
...
export function AssignKnowledgeBaseDialog({ isOpen, onClose, onAssign }: AssignKnowledgeBaseDialogProps) {
  ...
  const handleAssign = () => {
    if (knowledgeBaseName && (uploadedFiles.length > 0 || knowledgeBaseDescription)) {
      const newKnowledgeBase = {
        id: Math.random().toString(36).substr(2, 9),
        name: knowledgeBaseName,
        description: knowledgeBaseDescription || `${uploadedFiles.length} file(s) uploaded`,
        dateAdded: new Date(),
      }
      onAssign(newKnowledgeBase)
      toast({
        title: "Knowledge Base Created",
        description: `${knowledgeBaseName} has been successfully created.`,
      })
      onClose()
    } else {
      toast({
        title: "Error",
        description: "Please provide a name and either a description or upload files.",
        variant: "destructive",
      })
    }
  }
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md bg-[#09090b] text-white">
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="kb-description" className="text-sm font-medium text-gray-200">
        ...
        <Button
          ...
          disabled={!knowledgeBaseName || (uploadedFiles.length === 0 && !knowledgeBaseDescription)}
        >
